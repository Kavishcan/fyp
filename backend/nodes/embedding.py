...
dimensionality regardless of vocabulary growth, and both the coordinator and a
standalone node process can run it without the optional
torch/sentence-transformers dependencies. This is NOT the research pipeline's
intended embedder — swap in a real sentence-embedding model before any
leakage or routing-quality result is reported. The research code itself never
hardcodes an embedder (see profile.py, simulator.py); this class exists only
because something runnable is needed by default.

Lives in `nodes/` (not `api/`) because a real MCP node server
(`nodes/mcp_server.py`) is its own standalone process and must not import the
coordinator's API layer — `backend/api/embedder.py` re-exports this for
